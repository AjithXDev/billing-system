...
function showFilteredItems(filter) {
  ...
  listEl.innerHTML = items.map(function(p){ var badge = ''; if(_activeFilter === 'low' || (_activeFilter === 'all' && p.quantity < 10)) badge = '<span style="font-size:9px;background:rgba(239,68,68,0.15);color:var(--red);padding:2px 8px;border-radius:20px;margin-left:8px">LOW</span>'; var extra = ''; if(_activeFilter === 'expiry' && p.expiry_date) { var ed = new Date(p.expiry_date).toLocaleDateString('en-US',{month:'short',day:'numeric'}); extra = '<div style="font-size:11px;color:var(--text-s)">Expires: <span style="font-weight:bold;color:#f59e0b">'+ed+'</span></div>'; } else if(_activeFilter === 'dead') { extra = '<div style="font-size:11px;color:var(--text-s)">Stock: '+p.quantity+' &middot; No sales 30d</div>'; } else { extra = '<div style="font-size:11px;color:var(--text-s)">'+(p.category_name||'')+(p.quantity!==undefined?' &middot; Stock: '+p.quantity:'')+' </div>'; } var priceColor = _activeFilter === 'dead' ? 'var(--orange)' : 'var(--green)'; return '<div class="item"><div><div style="font-weight:800">'+p.name+badge+'</div>'+extra+'</div><div style="color:'+priceColor+';font-weight:800">Rs '+p.price+'</div></div>'; }).join('');
}
...
async function renderItems(){
  ...
  try { var thirtyAgo = new Date(); thirtyAgo.setDate(thirtyAgo.getDate()-30); thirtyAgo.setHours(0,0,0,0); var recentInvs = await live('invoices?select=local_id&created_at=gte.'+thirtyAgo.toISOString()); var activeIds = {}; recentInvs.forEach(function(i){ activeIds[i.local_id]=1; }); var recentItems = await live('invoice_items?select=product_id,invoice_id&limit=5000'); var soldPids = {}; recentItems.forEach(function(i){ if(activeIds[i.invoice_id]) soldPids[i.product_id]=1; }); _deadList = all.filter(function(p){ return p.quantity > 0 && !soldPids[p.local_id]; }); var dc = document.getElementById('dead-count'); if(dc) dc.textContent = _deadList.length; } catch(ex){ _deadList = []; }
...
var _currentTab = sessionStorage.getItem('iva_tab') || 'overview';
function render(){
